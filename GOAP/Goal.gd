...
## The highest value that can be retrieved from this priority. If "priority" is greater than priority_ceiling, priority_ceiling will be returned.
@export_range(MIN_PRIORITY, MAX_PRIORITY, 1) var priority_ceiling := 10 : set = set_priority_ceiling
## The default priority assigned to the goal. Can be left static or changed at runtime by overriding the method, "get_priority".
...
## This function is intended to be overriden for dynamic priority creation.
...
func is_valid(agent: Agent) -> bool:
	push_error("\"is_valid\" returns false by default and must be overriden by all Goal resources")
	return false
